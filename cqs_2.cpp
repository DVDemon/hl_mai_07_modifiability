...
auto main() -> int
{
    ...
    if (store.Read(2) == std::string("Message 2"))
        std::cout << "Added" << std::endl;
    store.Save(1, "Message 1");
    if (store.Read(1) == std::string("Message 1"))
        std::cout << "Added" << std::endl;
    store.Save(0, "Message 0");
    if (store.Read(0) == std::string("Message 0"))
        std::cout << "Added" << std::endl;
    store.Save(3, "Message New");
    if (store.Read(3) == std::string("Message New"))
        std::cout << "Added" << std::endl;

    try
    {
        for (int i = 0; i < 10; i++)
            std::cout << store.Read(i) << std::endl;
    }
    catch (std::string ex)
    {
